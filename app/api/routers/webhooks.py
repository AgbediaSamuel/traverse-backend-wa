...
def verify_webhook_signature(payload: bytes, signature: str, secret: str, msg_id: str = None, timestamp: str = None) -> bool:
    """
...
async def handle_clerk_webhook(request: Request):
    ...
    print("⚠️ TEMPORARILY SKIPPING SIGNATURE VERIFICATION FOR TESTING")
    # if not verify_webhook_signature(body, signature, CLERK_WEBHOOK_SIGNING_SECRET, msg_id, timestamp):
    #     raise HTTPException(
...
async def handle_user_deleted(user_data: Dict[str, Any]):
    ...
    try:
        ...
        if user:
            ...
            print(f"⚠️ User not found for deletion: {clerk_user_id}")
